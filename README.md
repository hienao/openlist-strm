...
### v1.0.11 (2025-01-15) 数据上报部分请关注下，如不需要请自行关闭
#### ✨ 新功能
- 🔍 **数据上报系统**: 新增匿名使用数据统计功能，帮助改进产品体验（可在设置中关闭）
- 📊 **日志管理系统**: 支持日志级别配置和保留天数设置，提供更灵活的日志管理
- 🆔 **容器实例管理**: 新增容器实例ID生成和获取功能，支持多实例部署识别
- 🤖 **AI刮削优化**: 添加正则匹配预处理，减少AI token消耗，提升刮削效率

#### 🛠️ 功能优化
- 🎨 **设置界面改进**: 新增数据上报开关、日志级别和保留天数配置选项
- 🧹 **日志清理增强**: 日志清理任务集成应用使用统计，优化清理逻辑
- ⚙️ **系统配置扩展**: 支持数据上报功能的开关控制和配置检查
- 📝 **代码质量提升**: 全面使用logger替换console输出，优化日志级别处理

#### 🔧 技术改进
- 🤖 **BMAD核心集成**: 添加完整的AI代理团队配置和开发文档模板
- 🏗️ **架构优化**: 新增DataReportService、LogConfigService等核心服务组件
...
#### ⚠️ 兼容性说明
- 日志级别变更需要重启应用后生效
- 数据上报功能默认开启，可在设置中关闭(仅上报容器id及刮削文件信息用于优化匹配逻辑，如有疑问可查看相关代码)
